...
INSERT INTO TBL_BOOK (bcode,title,writer,publisher,pdate) VALUES ('B1102','스트라이크 던지기','박철순','KBO','2020-11-10');
-- 2) 반납된 도서의 연체일수를 계산하여 delay_days 컬럼값을 update 합니다.
UPDATE TBL_BOOKRENT SET DELAY_DAYS = RETURN_DATE - EXP_DATE WHERE RETURN_DATE IS NULL;
...
SELECT name,title,exp_date FROM BOOK_MEMBER bm, TBL_BOOK tb, TBL_BOOKRENT tb2
...
WHERE sysdate > EXP_DATE AND RETURN_DATE IS NULL;
